...
@section('content')
    <div class="page-content">
        <nav class="page-breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ url('kas') }}">Kas</a></li>
                    ...
                    <div class="card-body">
                        @if (!empty($main))
                            <form id="signupForm" class="row" method="post"
                                action="{{ url('kas-update') . '/' . $main['id'] }}">
                                @csrf
                                @include('layouts.partial.alert')
                                <div class="mb-3 col-lg-6">
                                    <label class="form-label">Jenis Kas</label>
                                    <select class="js-example-basic-single form-select" data-width="100%" name="jenis">
                                        <option value="0">Pilih Jenis Kas</option>
                                        <option value="1" {{ $main['keluar'] == 0 ? 'selected' : '' }}>Masuk</option>
                                        <option value="2" {{ $main['keluar'] == 1 ? 'selected' : '' }}>Keluar</option>
                                    </select>
                                </div>
                                <div class="mb-3 col-lg-6">
                                ...
                                <div class="mb-3 col-lg-12">
                                    ...
                                    <textarea class="form-control" id="exampleFormControlTextarea1" name="uraian" rows="3"> {{ $main['uraian'] }}</textarea>
                                </div>
                                <div class="mb-3 col-lg-6">
                                    <label for="name" class="form-label">Jumlah</label>
                                    <div class="input-group">
                                        <span class="input-group-text">Rp.</span>
                                        <input id="name" class="form-control" name="banyaknya" type="text"
                                            value="{{ $main['banyaknya'] }}">
                                    </div>
                                </div>
                                <div class="mb-3 col-lg-6">
                                    ...
                                    <div class="input-group">
                                        <span class="input-group-text">Rp.</span>
                                        <input id="name" class="form-control" name="harga_satuan" type="text"
                                            value="{{ $main['harga_satuan'] }}">
                                    </div>
                                </div>
